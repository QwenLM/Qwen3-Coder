...
class Qwen3NextExperts(nn.Module):
    def __init__(self, config, layer_idx=0, offload_folder=None):
        ...
        if offload_folder:
            ...
            # Load Scales (Kept on GPU in BF16)
            self.gate_up_scale = self._load_scale_tensor(self.gate_up_path, f"{self.gate_up_key}_scale_inv",
                                                         device="cuda")
            ...
            # Header Metadata (Absolute Offsets)
            ...
            self.gate_up_weight_shape = (
                self.gate_up_meta['shape'][0] // self.num_experts, self.gate_up_meta['shape'][1])
            self.down_weight_shape = (self.down_meta['shape'][0] // self.num_experts, self.down_meta['shape'][1])

            self.gu_bytes = self.gate_up_weight_shape[0] * self.gate_up_weight_shape[1]
            self.d_bytes = self.down_weight_shape[0] * self.down_weight_shape[1]
            ...
            self.cache_up = {}  # Tier 1 (GPU)
            self.cache_down = {}  # Tier 1 (GPU)
            self.cache_up2 = {}  # Tier 2 (RAM Pinned)
            self.cache_down2 = {}  # Tier 2 (RAM Pinned)

            # Frequency Stats for LFU
            self.usage_stats = Counter()

            # Pre-allocate Pinned Landing Buffers (uint8 for NumPy compatibility)
            self.gu_pinned_buf = torch.empty(self.gu_bytes, dtype=torch.uint8, pin_memory=True)
            self.d_pinned_buf = torch.empty(self.d_bytes, dtype=torch.uint8, pin_memory=True)
    ...
    def _read_header(self, path, key):
        with open(path, "rb") as f:
            ...
            meta = header[key]
            meta['data_start'] = 8 + h_len + meta['data_offsets'][0]
            return meta

    def _apply_block_scale(self, weight, scale):
        out_dim, in_dim = weight.shape
        s_rows, s_cols = scale.shape
        # View weight in blocks matching scale shape
    ...
    def _evict_lfu(self):
        """Evicts the least frequently used expert from GPU to RAM."""
        # Find victim based on usage_stats
        victim_idx = min(self.cache_up.keys(), key=lambda k: self.usage_stats[k])

        v_gu = self.cache_up.pop(victim_idx)
        v_d = self.cache_down.pop(victim_idx)
        if not self.use_ram:
            return

        # Handle RAM (Tier 2) capacity
        if len(self.cache_up2) >= self.max_ram_cache:
            # Simple LFU for RAM as well
            ram_victim = min(self.cache_up2.keys(), key=lambda k: self.usage_stats[k])
            self.cache_up2.pop(ram_victim)
            self.cache_down2.pop(ram_victim)

        # Move to RAM as uint8 (pinned)
        self.cache_up2[victim_idx] = v_gu.view(torch.uint8).to("cpu", non_blocking=True).pin_memory()
        self.cache_down2[victim_idx] = v_d.view(torch.uint8).to("cpu", non_blocking=True).pin_memory()

    def _load_from_disk(self, idx, f_gu, f_d):
        """Disk -> Pinned Buffer -> GPU"""
        # Read Gate Up
        f_gu.seek(self.gate_up_meta['data_start'] + (idx * self.gu_bytes))
        f_gu.readinto(self.gu_pinned_buf.numpy().data)
        w_gu = self.gu_pinned_buf.to("cuda", non_blocking=True).view(self.view_dtype).view(
            self.gate_up_weight_shape).clone()

        # Read Down
        f_d.seek(self.down_meta['data_start'] + (idx * self.d_bytes))
        f_d.readinto(self.d_pinned_buf.numpy().data)
        w_d = self.d_pinned_buf.to("cuda", non_blocking=True).view(self.view_dtype).view(self.down_weight_shape).clone()
    ...
    def forward(self, hidden_states, top_k_index, top_k_weights):
        ...
        with open(self.gate_up_path, "rb") as f_gu, open(self.down_path, "rb") as f_d:
            for i, idx in enumerate(active_experts):
                ...
                if idx in self.cache_up:
                    ...
                    if idx in self.cache_up2:
                        w_gu_raw = self.cache_up2.pop(idx).to("cuda", non_blocking=True).view(self.view_dtype).view(
                            self.gate_up_weight_shape)
                        w_down_raw = self.cache_down2.pop(idx).to("cuda", non_blocking=True).view(self.view_dtype).view(
                            self.down_weight_shape)
                    else:
                        w_gu_raw, w_down_raw = self._load_from_disk(idx, f_gu, f_d)

                    if not self.use_only_ssd:
                        self.cache_up[idx] = w_gu_raw
                        self.cache_down[idx] = w_down_raw

                scale_g = self.gate_up_scale[
                          idx * self.gu_scale_rows_per_expert: (idx + 1) * self.gu_scale_rows_per_expert]
                scale_d = self.down_scale[idx * self.d_scale_rows_per_expert: (idx + 1) * self.d_scale_rows_per_expert]

                w_gu = self._apply_block_scale(w_gu_raw, scale_g)
                w_down = self._apply_block_scale(w_down_raw, scale_d)
