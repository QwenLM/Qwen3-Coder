...
class FP8Linear(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(init_weight.to(torch.float8_e4m3fn), requires_grad=False)
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.weight.device != input_device:
            weight_bf16 = self.weight.to(input_device).to(comp_dtype)
        else:
...
class Qwen3_5MoeTextRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        device_type = x.device.type if isinstance(x.device.type, str) and x.device.type != "mps" else "cpu" # else "cpu"
        with maybe_autocast(device_type=device_type, enabled=False):  # Force float32
            freqs = (inv_freq_expanded.float().to("cuda") @ position_ids_expanded.float()).transpose(2, 3) # .to("cuda")
...
class Qwen3_5MoeGatedDeltaNet(nn.Module):
    def __init__(self, config: Qwen3_5MoeConfig, layer_idx: int):
        ...
        self.out_proj = FP8Linear(self.value_dim, self.hidden_size, bias=False)
        ...
        self.in_proj_qkv = FP8Linear(self.hidden_size, self.key_dim * 2 + self.value_dim, bias=False)
        self.in_proj_z = FP8Linear(self.hidden_size, self.value_dim, bias=False)
        self.in_proj_b = FP8Linear(self.hidden_size, self.num_v_heads, bias=False)
        self.in_proj_a = FP8Linear(self.hidden_size, self.num_v_heads, bias=False)
...
class Qwen3_5MoeExperts_(nn.Module):
...
class DummyProj(nn.Module):
    """Dummy projection to satisfy PyTorch module structure without consuming RAM."""
...
class Qwen3_5MoeExperts(nn.Module):
    def __init__(self, config, layer_idx=0, offload_folder=None):
        ...
        #7 / 20
        self.max_gpu_cache = 7
        self.max_ram_cache = 20
    ...
    def _initialize_fast_io(self):
        ...
        layer_prefix = f"model.language_model.layers.{self.layer_idx}.mlp.experts"
            ...
            with safe_open(fpath, framework="pt", device="cuda") as sf:
                for key in sf.keys():
                    if key.startswith(layer_prefix) and "scale" in key:
                        parts = key.split('.')
                        exp_idx, proj = int(parts[6]), parts[7]
        ...
        # RAM Pool Tracking
        if self.use_ram:
            self.ram_pool = torch.empty((self.max_ram_cache, self.total_bytes), dtype=torch.uint8, pin_memory=self.pin_memory)
...
class Qwen3_5MoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        self.shared_expert_gate = FP8Linear(config.hidden_size, 1, bias=False) # torch.nn.Linear
    ...
    def forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        expert_output = self.experts(hidden_states_reshaped, selected_experts, routing_weights) # 99.6
...
class Qwen3_5MoeDecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states = self.mlp(hidden_states) # 98.1%
...
class Qwen3_5MoeVisionMLP(nn.Module):
    def __init__(self, config):
        ...
        self.linear_fc1 = FP8Linear(self.hidden_size, self.intermediate_size, bias=True)
        self.linear_fc2 = FP8Linear(self.intermediate_size, self.hidden_size, bias=True)
        self.act_fn = ACT2FN[config.hidden_act]

    @torch.inference_mode() #unused
...
class Qwen3_5MoeVisionPatchMerger(nn.Module):
    def __init__(self, config: Qwen3_5MoeVisionConfig, use_postshuffle_norm=False) -> None:
        ...
        self.linear_fc1 = FP8Linear(self.hidden_size, self.hidden_size)
        self.act_fn = nn.GELU()
        self.linear_fc2 = FP8Linear(self.hidden_size, config.out_hidden_size)
...
class Qwen3_5MoeVisionAttention(nn.Module):
    def __init__(self, config: Qwen3_5MoeVisionConfig) -> None:
        ...
        self.qkv = FP8Linear(self.dim, self.dim * 3, bias=True)
        self.proj = FP8Linear(self.dim, self.dim)
        self.scaling = self.head_dim**-0.5
        self.config = config
...
class Qwen3_5MoeForCausalLM(Qwen3_5MoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = FP8Linear(config.hidden_size, config.vocab_size, bias=False)
...
class Qwen3_5MoeForConditionalGeneration(Qwen3_5MoePreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.lm_head = FP8Linear(config.text_config.hidden_size, config.text_config.vocab_size, bias=False)
